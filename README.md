...
| 기준 | 점수(상한) | 근거 | 채점 방식 |
| --- | --- | --- | --- |
| 실행 검증 (execution) | **40** | Docker 실제 실행 | **결정적(빌드·테스트)** |
| 기능 구현·완성도 (functionality) | 5 | 소스 + 실행/테스트 | AI(절대 앵커, **동작 기능 수** 강조) |
| 코드 품질 (code_quality) | 3 | 소스파일 | AI(절대 앵커) |
| README·문서화 (documentation) | 2 | README/문서 | AI(절대 앵커) |

- **기본 점수 만점 = 50점** (실행 40 + AI 루브릭 10). 여기에 가산점 **최대 50점**(Azure 25 + MS 스택 25)이 더해집니다.
...
## Azure 배포 가산점 (15–25점)
- **Azure 배포 증거**가 있으면 **20점**, 제출한 **Azure 배포 URL이 실제 응답(live)** 하면 **25점**을 더합니다(종합 상한 100).
- 감지 신호: `azure.yaml`(azd), `*.bicep`/`infra/`, GitHub Actions Azure 배포/로그인,
  `azurewebsites.net`·`azurecontainerapps.io` 등 호스트명, ACR 등.

## Microsoft AI 스택 가산점 (10–25점)
- 다음 기술을 사용하면 **사용한 구성요소 수에 따라** 가산점을 줍니다(1개 10점, 2개 20점, 3개 이상 25점):
...
| `EXECUTION_WEIGHT` | "실행 검증" 항목 점수 상한 (기본 40) |
| `EXECUTION_TIMEOUT` | 샌드박스 실행 제한 시간(초, 기본 240) |
| `AZURE_BONUS_MIN` / `AZURE_BONUS_MAX` | Azure 배포 가산점 (감지 15 / 라이브 25) |
| `MS_STACK_BONUS_MIN` / `MAX` / `PER` | MS AI 스택 가산점 (구성요소당 10, 10~25) |
