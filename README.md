...
3. **실행 검증**: Docker 샌드박스에서 `install → build → test` 실행 (결정적)
4. **AI 심사**: gpt-5.4가 digest + 실행 결과를 근거로 루브릭 항목을 0–10 채점 (JSON 구조화 출력)
5. **점수 합산**: `(점수/10 × 상한)` 합(기본) + 가산점(Azure·MS) → 종합 `min(100, …)`, 리더보드 반영
...
| ☁️ Azure 배포 가산점 | **+20** | 배포 증거 | 결정적(감지 시 가산) |
| 🧩 MS AI 스택 가산점 | **+20** | 사용 기술 | 결정적(사용 시 가산) |

- **기본 60점**(실행 20 + 기능 20 + 문서 20) + **가산 최대 40점**(Azure 20 + MS 스택 20) = **총 100점**.
...
## AI는 어떻게 채점하나 (AI 루브릭)
"AI 루브릭"은 gpt-5.4가 **저장소 산출물(소스·설정·README)만 읽고** 채점하는 항목(기능 구현·완성도, 문서화)입니다.
- 각 항목에 **0–10 절대 기준(앵커)** 을 주고, *"다른 팀과 비교하지 말고 관찰된 사실로만 판단하며, 어떤 파일/코드를 봤는지 근거를 인용"* 하도록 지시합니다.
- `temperature 0.1` + JSON 스키마 구조화 출력으로 일관성·파싱 안정성을 확보합니다.
- "실제로 동작하는가"는 추측에 맡기지 않고 **실행 검증(결정적)** 으로 분리하며, 그 결과를 AI 심사에 근거로 전달합니다.
- 한계: LLM 판단이라 완전한 결정성은 아니며(재현성에 약간의 변동), 대용량 저장소는 일부만 봅니다(절단).
...
## Azure 배포 가산점 (20점, 고정)
- **Azure 배포 증거**가 감지되면 **+20점**을 더합니다(종합 상한 100). 제출한 Azure 배포 URL의 실제 응답(live) 여부도 신호로 활용합니다.
- 감지 신호: `azure.yaml`(azd), `*.bicep`/`infra/`, GitHub Actions Azure 배포/로그인,
  `azurewebsites.net`·`azurecontainerapps.io` 등 호스트명, ACR 등.

## Microsoft AI 스택 가산점 (20점, 고정)
- 다음 기술을 하나 이상 사용하면 **+20점**을 줍니다:
...
| `AZURE_BONUS_MIN` / `AZURE_BONUS_MAX` | Azure 배포 가산점 (감지 시 20, 고정) |
| `MS_STACK_BONUS_MIN` / `MAX` / `PER` | MS AI 스택 가산점 (사용 시 20, 고정) |
...
| 메서드 | 경로 | 설명 |
| --- | --- | --- |
| GET | `/api/health` | 상태 + 채점 구성(가중치·가산점 범위) |
...
Azure/MS 가산점 탐지, 파이프라인, 인증을 커버합니다(외부 호출은 모킹).
