...
| 기준 | 점수(상한) | 근거 | 채점 방식 |
| --- | --- | --- | --- |
| 실행 검증 (execution) | **50** | Docker 실제 실행 | **결정적(빌드·테스트)** |
| 기능 구현·완성도 (functionality) | 15 | 소스 + 실행/테스트 | AI(절대 앵커, **동작 기능 수** 강조) |
| 코드 품질 (code_quality) | 8 | 소스파일 | AI(절대 앵커) |
| README·문서화 (documentation) | 7 | README/문서 | AI(절대 앵커) |

- **기본 점수 만점 = 80점** (실행 50 + AI 루브릭 30). 여기에 아래 가산점이 더해집니다.
- **종합 = min(100, 기본 + Azure 가산 + MS 스택 가산)**.
- 주관적 '아이디어 참신함'은 저장소만으로 객관 판단이 어려워 루브릭에서 제외했습니다.
- ⚠️ 실행 검증은 **Node·Python·Go** 만 자동 실행됩니다. 그 외 스택(.NET/Java 등)은 실행 점수 0이 되어
  불리하니 필요하면 실행기 확장이 필요합니다.
...
## Azure 배포 가산점 (20–40점)
- **Azure 배포 증거**가 있으면 **20점**, 제출한 **Azure 배포 URL이 실제 응답(live)** 하면 **40점**을 더합니다(종합 상한 100).
- 감지 신호: `azure.yaml`(azd), `*.bicep`/`infra/`, GitHub Actions Azure 배포/로그인,
  `azurewebsites.net`·`azurecontainerapps.io` 등 호스트명, ACR 등.

## Microsoft AI 스택 가산점 (15–45점)
- 다음 기술을 사용하면 **사용한 구성요소 수에 따라** 가산점을 줍니다(1개 15점, 2개 30점, 3개 이상 45점):
...
| `EXECUTION_WEIGHT` | "실행 검증" 항목 점수 상한 (기본 50) |
| `EXECUTION_TIMEOUT` | 샌드박스 실행 제한 시간(초, 기본 240) |
| `AZURE_BONUS_MIN` / `AZURE_BONUS_MAX` | Azure 배포 가산점 (감지 20 / 라이브 40) |
| `MS_STACK_BONUS_MIN` / `MAX` / `PER` | MS AI 스택 가산점 (구성요소당 15, 15~45) |
